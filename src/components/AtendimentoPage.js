...
import { fetchAtendimentos, createAtendimento, updateAtendimento, deleteAtendimento, getAtendimentoById, fetchAtendimentosPage, fetchAtendimentosFilterPage } from '../services/AtendimentoService';
...
function AtendimentoPage() {
    ...
    const handleDelete = async (atendimentoId) => {
        try {
            const confirmation = window.confirm("Tem certeza de que deseja excluir esto atendimento?");
            if (confirmation) {
            ...
            alert('Erro ao excluir o atendimento. Por favor, tente novamente mais tarde.');
        }
    };
    ...
    const handleUpdate = async (atendimentoToUpdate) => {
        try {
            ...
            console.error("Houve um erro ao atualizar o atendimento:", error.response?.data?.erro);
            if (error.response?.data?.erro) {
                alert(`Erro ao atualizar o atendimento. ${error.response.data.erro}.`);
                
            }
            else {
                alert('Erro ao atualizar o atendimento. Por favor, tente novamente mais tarde.');
            }
        }
    };
    ...
    const handleEditAtendimento = async (atendimento) => {
        ...
        response.data.dataAtendimento = convertBackendDateToFrontend(response.data.dataAtendimento);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            console.error("Houve um erro ao criar o atendimento:", error.response.data.erro);
    ...
    const handleSubmitProduto = () => {
        ...
        atendimento.produtoAtendimento = produtos;
    ...
    return (
        <div>
            <div className="d-flex justify-content-between mb-3">
                ...
                <Button className="btn btn-primary btn-lg" onClick={handleOpenModal}>Cadastrar Atendimento</Button>
            </div>
                ...
                <tbody>
                    {allAtendimento.map(atendimento => (
                        <tr key={atendimento.id}>
                            ...
                            <td>{new Date(atendimento.dataAtendimento).toLocaleDateString()}</td>
                            <td>
                                <button className="btn btn-warning btn-sm me-2" onClick={() => handleEditAtendimento(atendimento)}>Alterar</button>
                                <button className="btn btn-danger btn-sm" onClick={() => handleDelete(atendimento.id)}>Deletar</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                ...
                <Modal.Body>
                    <div>
                        <label>Tutor:</label>
                        <input type="number" className="form-control" name="tutor-id" value={atendimento.tutor?.id} onChange={handleInputChange} />
                    </div>
                    <div>
                        <label>Animal:</label>
                        <input type="number" className="form-control" name="animal-id" value={atendimento.animal?.id} onChange={handleInputChange} />
                    </div>
                    <div>
                        <label>Empregado:</label>
                        <input type="number" className="form-control" name="empregado-id" value={atendimento.empregado?.id} onChange={handleInputChange} />
                    </div>
                    <div>
                        <label>Data atendimento:</label>
                        <input type="date" className="form-control" name="dataAtendimento" value={atendimento.dataAtendimento} onChange={handleInputChange} />
                    </div>
                    <div >
                        <label>Houve internação:</label>
                        <input type="checkbox" className="form-check-input" name="houveInternacao" checked={atendimento.houveInternacao} onChange={e => setAtendimento(prev => ({ ...prev, houveInternacao: e.target.checked }))} />
                    </div>
                </Modal.Body>
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={(event) => atendimento.id ? handleUpdate(atendimento) : handleSubmit(event)}>
            ...
            <Modal show={showModalProduto} onHide={() => setShowModalProduto(false)}>
                ...
                <Modal.Body>
                    <div>
                        ...
                        <select className="form-control" name="tipoProduto" value={produtoAtendimento.tipoProduto} onChange={handleInputProdutoChange}>
                    ...
                    <div>
                        <label>Produto:</label>
                        <input type="number" className="form-control" name="produto-id" value={produtoAtendimento.produto?.id} onChange={handleInputProdutoChange} />
                    </div>
                    {produtoAtendimento.tipoProduto === 'ATENDIMENTO' ?
                        <div>
                            <label>Data liberação:</label>
                            <input type="date" className="form-control" name="dataLiberacao" value={produtoAtendimento.dataLiberacao} onChange={handleInputProdutoChange} />
                        </div> : null}
                    {produtoAtendimento.tipoProduto === 'ATENDIMENTO' ?
                        <div>
                            <label>Data internação:</label>
                            <input type="date" className="form-control" name="dataInternacao" value={produtoAtendimento.dataInternacao} onChange={handleInputProdutoChange} />
                        </div> : null}
                    {produtoAtendimento.tipoProduto === 'PROCEDIMENTO' ?
                        <div>
                            <label>Data procedimento:</label>
                            <input type="date" className="form-control" name="data" value={produtoAtendimento.data} onChange={handleInputProdutoChange} />
                        </div> : null}
                    {produtoAtendimento.tipoProduto === 'MATERIAL' || produtoAtendimento.tipoProduto === 'REMEDIO' ?
                        <div>
                            <label>Quantidade:</label>
                            <input type="number" className="form-control" name="quantidade" value={produtoAtendimento.quantidade} onChange={handleInputProdutoChange} />
                        </div> : null}
                    <div>
                        <label>Valor:</label>
                        <input type="number" className="form-control" name="valor" value={produtoAtendimento.valor} onChange={handleInputProdutoChange} />
                    </div>
                    <div>
                        <label>Desconto:</label>
                        <input type="number" className="form-control" name="desconto" value={produtoAtendimento.desconto} onChange={handleInputProdutoChange} />
                    </div>
                    <div>
                        <label>Valor total:</label>
                        <input type="number" className="form-control" name="valorTotal" value={produtoAtendimento.valorTotal} onChange={handleInputProdutoChange} disabled />
                    </div>
                </Modal.Body>
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={(event) => handleSubmitProduto(event)}>
                        {atendimento.id ? 'Atualizar' : 'Salvar'}
                    </Button>
                </Modal.Footer>
            </Modal>
