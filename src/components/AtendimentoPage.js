...
function AtendimentoPage() {
  ...
  const [dataAtendimentoFilter, setDataAtendimentoFilter] = useState("");
  const [dataAtendimentoFinalFilter, setDataAtendimentoFinalFilter] =
    useState("");
  ...
  function limparFiltrosEspeciais() {
    setDataAtendimentoFilter("");
    setDataAtendimentoFinalFilter("");
  }
  ...
  const handleInputProdutoChange = (event) => {
    ...
    setProdutoAtendimento((prevAtendimento) => ({
      ...prevAtendimento,
      valorTotal: getValorTotalProduto(prevAtendimento),
    }));
  };
  ...
  function handleFecthEspecialFilterList() {
    if (dataAtendimentoFilter && dataAtendimentoFinalFilter) {
      const filter = `dataAtendimento%2Bbetween%2B${encodeURIComponent(
        dataAtendimentoFilter
      )}%2B${encodeURIComponent(dataAtendimentoFinalFilter)}`;
      fetchAtendimentosFilterPage(`?filter=${filter}`)
        .then((response) => {
          setAllAtendimento(response.data.content);
          setTotalItens(response.data.totalElements);
        })
  ...
  const handleEditAtendimento = async (atendimento) => {
    ...
    console.log(response.data.produtoAtendimento);
    for (const produtoAtendimento of response.data.produtoAtendimento) {
      produtoAtendimento.data = convertBackendDateToFrontend(
        produtoAtendimento.data
      );
      produtoAtendimento.dataInternacao = convertBackendDateToFrontend(
        produtoAtendimento.dataInternacao
      );
      produtoAtendimento.dataLiberacao = convertBackendDateToFrontend(
        produtoAtendimento.dataLiberacao
      );
    }
  ...
  const handleEditProdutoAtendimento = (produto, indexProduto) => {
    ...
    produto.valorTotal = produto.valorTotal
      ? produto.valorTotal
  ...
  const getValorTotalProduto = (produto) => {
    let valorTotal = produto.valor;
    if (produto.tipoProduto === "INTERNACAO") {
      valorTotal = produto.valor * produto.quantidadeDias;
    } else if (
      produto.tipoProduto === "MATERIAL" ||
      produto.tipoProduto === "REMEDIO"
    ) {
      valorTotal = produto.valor * produto.quantidade;
    }
    valorTotal = valorTotal - produto.desconto;
    return valorTotal;
  };
    ...
    <div>
          ...
          >
            ...
            <div className="p-2">
              ...
              <input
                ...
                value={dataAtendimentoFilter}
                onChange={(e) => setDataAtendimentoFilter(e.target.value)}
              />
              <br></br>
              <input
                type="date"
                className="form-control"
                value={dataAtendimentoFinalFilter}
                onChange={(e) => setDataAtendimentoFinalFilter(e.target.value)}
              />
            </div>
            ...
            <tbody>
              {atendimento.produtoAtendimento.map((produtoAtend, index) => (
                <tr key={produtoAtend.id}>
                  ...
                  <td>
                    {produtoAtend.produto.descricao
                      ? produtoAtend.produto.descricao.substring(0, 14)
                      : ""}
                  </td>
                  <td>
                    {produtoAtend.tipoProduto === "INTERNACAO"
                      ? "Internação"
                      : produtoAtend.tipoProduto === "PROCEDIMENTO"
                      ? "Procedimento"
                      : produtoAtend.tipoProduto === "REMEDIO"
                      ? "Remédio"
                      : "Material"}
                  </td>
      ...
      <Modal show={showModalProduto} onHide={() => setShowModalProduto(false)}>
        ...
        <Modal.Body>
          ...
          {produtoAtendimento.tipoProduto === "INTERNACAO" ? (
            <div>
